...
<form action="/notes/new" method="post" enctype="multipart/form-data" accept-charset="utf-8" id="newnote">
  ...
  <fieldset class="note-ui">
    ...
    <label for="syndication" accesskey="s">Syndication: </label>@foreach($syndication as $target)<input type="checkbox" name="{{ $target }}" id="{{ $target }}" checked="checked"> <label for="{{ $target }}">{{ $target }}</label><br>@endforeach
