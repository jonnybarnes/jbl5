...
<form action="/notes/new" method="post" enctype="multipart/form-data" accept-charset="utf-8" id="newnote">
  ...
  <fieldset class="note-ui">
    ...
    <label for="syndication" accesskey="s">Syndication: </label><ul>@foreach($syndication as $target)<li><input type="checkbox" name="syndicate-to={{ $target }}" id="{{ $target }}" checked="checked"> <label for="{{ $target }}">{{ $target }}</label></li>@endforeach</ul>
