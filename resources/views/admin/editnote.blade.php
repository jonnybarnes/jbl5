...
<form action="/admin/note/edit/{{ $id }}" method="post" accept-charset="utf-8">
  ...
  <fieldset class="note-ui">
    ...
    <label for="in-reply-to" accesskey="r">Reply-to: </label><input type="text" name="reply-to" id="reply-to" placeholder="in-reply-to-1 in-reply-to-2 …" tabindex="1" value="{{ $note->reply_to }}"><br>
    <label for="content" accesskey="n">Note: </label><textarea name="content" id="content" placeholder="Note" tabindex="2">{{ $note->note }}</textarea><br>
