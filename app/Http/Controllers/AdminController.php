...
class AdminController extends Controller {
	...
	{
		return view('adminwelcome', array('name' => $this->username));
	}
/*
	...
	public function newArticle()
	{
		...
		return View::make('newarticle', array('message' => $message));
	}

	public function listArticles()
	{
		$posts = Article::select('id', 'title', 'published')->where('deleted', '0')->orderBy('id', 'desc')->get();
		return View::make('listarticles', array('posts' => $posts));
	}
	...
	public function editArticle($id)
	{
		...
		return View::make('editarticle', array('id' => $id, 'post' => $post));
	}

	public function deleteArticle($id)
	{
		return View::make('deletearticle', array('id' => $id));
	}

	public function postNewArticle()
	{
		$title = Input::get('title');
		$titleurl = Slugify::slugify(strip_tags($title));
		$url = Input::get('url');
		$main = Input::get('main');
		...
		try{
			...
			if($unique) {
				...
				return Redirect::to('admin/blog/new')->withInput()->with('message', 'Duplicate title, please change');
			} else {
				//this isn't the error you're looking for
				throw $e;
			}
		}
		return View::make('newarticlesuccess', array('id' => $id, 'title' => $title));
	}
	...
	public function postEditArticle($id)
	{
		...
		return View::make('editarticlesuccess', array('id' => $id));
	}

	public function postDeleteArticle($id)
	{
		Article::where('id', $id)->update(array('deleted' => '1'));
		return View::make('deletearticlesuccess', array('id' => $id));
	}
